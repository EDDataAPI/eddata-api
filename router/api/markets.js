...
module.exports = (router) => {
  // Get info about a specific market (with /api prefix)
  ...
  // Get info about a specific market (without /api prefix)
  router.get('/v2/market/:marketId', async (ctx, next) => {
    const { marketId } = ctx.params
    const commodities = await dbAsync.get(`
      SELECT * FROM stations.stations s WHERE s.marketId = @marketId
      `, { marketId }
  ...
  // Get buy/sell orders for commodities at a specific market (with /api prefix)
  ...
  // Get buy/sell orders for commodities at a specific market (without /api prefix)
  router.get('/v2/market/:marketId/commodities', async (ctx, next) => {
    const { marketId } = ctx.params
    const commodities = await dbAsync.all(`
      SELECT
        c.commodityName,
  ...
  // Get buy/sell orders for a specific commodity at a specific market (with /api prefix)
  ...
  // Get buy/sell orders for a specific commodity at a specific market (without /api prefix)
  router.get('/v2/market/:marketId/commodity/name/:commodityName', async (ctx, next) => {
    const { marketId, commodityName } = ctx.params
