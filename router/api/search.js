...
module.exports = (router) => {
  // Search systems by name (with /api prefix)
  ...
  // Search systems by name (without /api prefix)
  router.get('/v2/search/system/name/:systemName', async (ctx, next) => {
    const { systemName } = ctx.params
    if (systemName.length < 1) {
      ctx.status = 406
      ctx.body = {
  ...
  // Search stations by name (with /api prefix)
  ...
  router.get('/v2/search/station/name/:stationName', async (ctx, next) => {
    const { stationName } = ctx.params
    if (stationName.length < 1) {
      ctx.status = 406
      ctx.body = {
