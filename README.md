...
**Important:** The API requires SQLite databases to be present in the data directory.

#### Option 1: Use EDData Collector (Recommended)

The databases are generated by [EDData Collector](https://github.com/EDDataAPI/eddata-collector):

```bash
# Run collector to generate databases
# See eddata-collector repository for setup instructions
```

#### Option 2: Mount Existing Databases

If you have existing databases, mount them to the correct location:
...
Required database files in `/app/data/`:
- `systems.db` - Star systems and coordinates
- `locations.db` - Points of interest
- `stations.db` - Stations and fleet carriers
- `trade.db` - Commodity trading data

**Without these databases, the API will fail to start with `SQLITE_ERROR`.**
