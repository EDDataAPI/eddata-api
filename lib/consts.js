...
// Container: respects EDDATA_DATA_DIR env var (set in docker-compose)
// Local: uses eddata-data directory
const defaultDataDir = isContainerEnv
  ? '/app/eddata-data'  // Container default - will be overridden by env var
  : path.join(__dirname, '../eddata-data')  // Local default
const defaultBackupDir = isContainerEnv
  ? '/app/eddata-backup'
  : path.join(__dirname, '../eddata-backup')
const defaultDownloadsDir = isContainerEnv
  ? '/app/eddata-downloads'
...
// Use environment variable first, then fall back to default paths
...
console.log('[CONFIG] EDDATA_DATA_DIR:', EDDATA_DATA_DIR)
console.log('[CONFIG] EDDATA_CACHE_DIR:', EDDATA_CACHE_DIR)
